...
function register() {
	...
	registerBlock(
		...
		{
			message0:
				"create block %1 id: %2 %3 text: %4 %5 type: %6 %7 block shape: %8 %9 allow Drop Anywhere: %10 %11 disable monitor: %12 %13 filter: %14 %15 hide from palette: %16 inputs: %17 %18 function: %19 %20",
			args0: [
				{
					type: "input_dummy",
				},
		...
		(block) => {
			...
			const ADAW = block.getFieldValue("ADAW");
			const DISABLEMONITOR = block.getFieldValue("DISABLEMONITOR");
			const FILTER = block.getFieldValue("FILTER");
			const HIDEFROMPALETTE = javascriptGenerator.valueToCode(
				block,
				"HIDEFROMPALETTE",
            ...
            text: \`${TEXT}\`,` +
				(TYPE != "COMMAND" && BLOCKSHAPE != "DEFAULT"
					? `blockShape: Scratch.BlockShape.${BLOCKSHAPE},\n`
					: ``) +
				(TYPE == "REPORTER"
					? `allowDropAnywhere: ${ADAW.toLowerCase()},`
					: ``) +
				(TYPE == "REPORTER" || TYPE == "BOOLEAN"
					? `disableMonitor: ${DISABLEMONITOR.toLowerCase()},\n`
					: ``) +
				(FILTER === "SPRITE"
	...
	//object
	registerBlock(
		`${categoryPrefix}createobject`,
		{
			message0:
				"create object %1 id: %2 %3 text: %4 %5 type: %6 %7 filter: %8 %9 hide from palette?: %10 function: %11 %12",
			args0: [
				{
					type: "input_dummy",
				},
			...
			const FUNC =
				TYPE === "BUTTON"
					? "Extension.prototype[`" +
					  ...
					  "`] = async (args, util) => { " +
					  javascriptGenerator.statementToCode(block, "FUNC") +
					  " };"
					: "";
	...
	registerBlock(
		...
		(block) => {
			const ID = block.getFieldValue("ID");
			const DATAURI = javascriptGenerator.valueToCode(
				block,
				"DATAURI",
				javascriptGenerator.ORDER_ATOMIC
			);
