...
function register() {
    ...
    // open group
    registerBlock(`${categoryPrefix}group`, {
        ...
        const LABEL = javascriptGenerator.valueToCode(block, 'LABEL', javascriptGenerator.ORDER_ATOMIC);
        const BLOCKS = javascriptGenerator.statementToCode(block, 'BLOCKS');
        const code = `console.group(${LABEL || ""})\n${BLOCKS} console.groupEnd()`;
        return `${code}\n`;
    })

    // collapsed group
    registerBlock(`${categoryPrefix}groupcollapsed`, {
        ...
        const LABEL = javascriptGenerator.valueToCode(block, 'LABEL', javascriptGenerator.ORDER_ATOMIC);
        const BLOCKS = javascriptGenerator.statementToCode(block, 'BLOCKS');
        const code = `console.groupCollapsed(${LABEL || ""})\n${BLOCKS} console.groupEnd()`;
        return `${code}\n`;
    })

    // eval block
    registerBlock(`${categoryPrefix}evalblock`, {
        message0: 'eval %1',
        args0: [
            {
                "type": "field_input",
