...
function register() {
	...
	registerBlock(
		...
		(block) => {
			const A = javascriptGenerator.valueToCode(
				block,
				"A",
				javascriptGenerator.ORDER_ATOMIC
			);
			const B = javascriptGenerator.valueToCode(
				block,
				"B",
				javascriptGenerator.ORDER_ATOMIC
			);
	...
	registerBlock(
		`${categoryPrefix}createdynamic`,
		{
			message0:
				"create dynamic menu %1 id: %2 %3 function: %4 allow inputs: %5",
			args0: [
				{
					type: "input_dummy",
				},
