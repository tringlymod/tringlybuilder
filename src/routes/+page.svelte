<script>
	...
	import ExtensionInfoModal from "$lib/MenuModals/ExtensionInfo.svelte";
	...
	import registerJSONBuilder from "../resources/blocks/JSONBuilder.js";
	registerJSONBuilder();

	import registerMap from "../resources/blocks/map.js";
	registerMap();

	import registerSet from "../resources/blocks/set.js";
	registerSet();
	...
	import registerScratchVM from "../resources/blocks/ScratchVM.js";
	registerScratchVM();
	...
	function downloadProject() {
		// generate file name
		let filteredProjectName = (
			extensionMetadata.fileName ??
	...
	function displayGeneratedCode(code) {
		...
		const highlighted = Prism.highlight(beautified, Prism.languages.javascript);
		return highlighted;
	}
	...
	function githubRepoLink() {
		window.open("https://github.com/tringlymod/tringlybuilder");
	}
</script>
...
<NavigationBar>
	...
	<NavigationButton on:click={githubRepoLink}>GitHub</NavigationButton>
	<NavigationDivider />
		...
		<div class="row-submenus">
			<div class="assetsWrapper">
				...
				<p>
					...
					in the block list.
					<br />
					These things are not required, so you can leave them empty if you do not
					need them.
				</p>
				<p>
					Documentation URL:
				...
				{#if extensionImageStates.icon.image}
					{#if extensionImageStates.icon.failed}
						<p class="warning">
							The extension icon is not an image, this may appear broken in the
							category list.
						</p>
					{/if}
					{#if !extensionImageStates.icon.square}
						<p class="warning">
							The image is not square, this may appear broken in the category
							list.
						</p>
					{/if}
				{/if}
			...
			<div class="row-subsubmenus">
				<div class="codeActionsWrapper">
					...
					<StyledButton
						on:click={() => {
							// copy code
							const code = beautifyGeneratedCode(lastGeneratedCode);
